...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        try {
            ...
            if ($stmt->execute()) {
                if (sendVerificationEmail($email, $verification_code)) {
                    $_SESSION['verifying_email'] = $email; // Simpan email di sesi
                    header("Location: ../verify/"); // Redirect ke halaman verifikasi
                    exit();
                } else {
                    header("Location: ../register/?error=email_send_failed");
                    exit();
                }
            } else {
    ...
    if ($action == 'verify') {
        $email = $_SESSION['verifying_email'] ?? ''; // Ambil email dari sesi
        $code = $_POST['code'];
        ...
        if ($result->num_rows > 0) {
            ...
            unset($_SESSION['verifying_email']); // Hapus email dari sesi setelah verifikasi
            header("Location: ../login/?status=verified");
            exit();
        } else {
            header("Location: ../verify/?error=invalid_code");
            exit();
        }
    }
        ...
        if ($user = $result->fetch_assoc()) {
            if (password_verify($password, $user['password_hash'])) {
                if ($user['is_verified'] == 1) {
                    ...
                    header("Location: ../verify/?status=unverified");
                    exit();
                }
            }
        }
